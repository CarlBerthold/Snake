...
function colision() {
  // get middle of the snake 
  const snakeMiddleX = snakeX + snakeWidth / 2;
  const snakeMiddleY = snakeY + snakeHeight / 2;
  // get middle of the obstacle
  const obstacleMiddleX = obstacleX + obstacleWidth / 2;
  const obstacleMiddleY = obstacleY + obstacleHeight / 2;
  // check if distance between the middle of obstacle and snake
  // is less than the width of the snake
  if (dist(snakeMiddleX, snakeMiddleY, obstacleMiddleX, obstacleMiddleY) < snakeWidth) {
    ...
    if (counter === 10) {
      alert('You made it 🙀 🥇 🚀');
      // stops the execution of the p5 loop
      noLoop();
    }
    // replace obstacle
